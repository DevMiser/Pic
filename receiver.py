...
try:
    ...
    print(f"ERROR: Importing display libs: {e}")
    sys.exit(1)
...
LORA_AIR_SPEED = 2400
RX_NODE_ADDRESS = 1 # This node's address (not used by module in Transparent mode)

# --- LCD Configuration & Layout (Same as previous v4) ---
...
try:
    FONT_PATH_PERCENT = "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf"; FONT_SIZE_PERCENT = 55
    percent_font = ImageFont.truetype(FONT_PATH_PERCENT, FONT_SIZE_PERCENT); print(f"[INFO] Font: {FONT_PATH_PERCENT}")
...
# --- Functions (initialize_lora, initialize_lcd, update_display, cleanup - Same as Receiver v4) ---
def initialize_lora():
    global node; print("[INFO] Initializing LoRa (v5 - Transparent)...")
    try:
        # Use v5 lora-code, enable RSSI, verbose=False for clean output
        node = lora_code.sx126x(
            serial_num=LORA_SERIAL_PORT, freq=LORA_FREQUENCY,
            addr=RX_NODE_ADDRESS, power=LORA_POWER, rssi=True, # Enable RSSI
            air_speed=LORA_AIR_SPEED, verbose=False # Set True for debug
        )
...
def initialize_lcd():
    ...
    try:
        ...
        rotated_img = img.rotate(90, expand=True); disp.ShowImage(rotated_img) # Use +90 rotation
        return True
...
def update_display(value):
    ...
    img=Image.new('RGB', (CANVAS_WIDTH, CANVAS_HEIGHT), COLOR_WHITE); draw=ImageDraw.Draw(img)
    percentage = value * 10; text = f"{percentage}%"
    if percent_font:
        try: bbox=draw.textbbox((CANVAS_WIDTH//2, TEXT_AREA_HEIGHT//2), text, font=percent_font, anchor="mm"); draw.text((bbox[0], bbox[1]), text, font=percent_font, fill=COLOR_BLACK)
        except Exception as e: print(f"[ERROR] Text draw error: {e}")
    for i in range(BAR_COUNT):
        coords=bar_coords[i]; color=bar_colors[i]; bar_idx_bot=BAR_COUNT-1-i
        draw.rectangle(coords, outline=COLOR_OUTLINE, width=1)
        if (bar_idx_bot + 1) <= value:
    ...
    try: rotated_img = img.rotate(90, expand=True); disp.ShowImage(rotated_img); print("[INFO] Display updated.") # Use +90 rotation
...
def cleanup():
    ...
    if node: try: node.close(); print("[INFO] LoRa released.")
             except Exception as e: print(f"[WARN] LoRa cleanup error: {e}")
    print("[INFO] Cleanup finished.")
# --- End Functions ---

def main():
    print("--- LoRa Receiver (v5 - Transparent Mode) ---")
    if not initialize_lora() or not initialize_lcd(): print("[FATAL] Init failed."); cleanup(); sys.exit(1)

    print("-" * 35)
    print(f"Listening: Addr={RX_NODE_ADDRESS}(N/A), Freq={LORA_FREQUENCY}, Speed={LORA_AIR_SPEED}")
    print("Mode: Transparent, Orientation: Vertical")
    print("Press Ctrl+C to exit.")
    print("-" * 35)

    print("[INFO] Setting initial display: 9 (90%)")
    update_display(9)

    while True:
        try:
            # Use receive method from lora-code.py v5 (Transparent)
            payload_bytes, rssi = node.receive()

            if payload_bytes is not None:
                # Should receive just the number string bytes
                try:
